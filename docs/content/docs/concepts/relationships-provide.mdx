...
*   **Definition:**
    *   What a Kit *can* provide is defined in the `provide` section of its `kit.yaml` file ([Kit YAML Reference](/kit-development/kit-yaml)). This declares the potential Actions, Instructions, or Workspace access it's willing to share.
...
1.  **`ACTION`**
    *   **What it shares:** Specific executable functions (Kit Actions) defined in the *Provider* Kit's `provide.actions` section.
    *   **How it works:** When Module A provides an `ACTION` to Module B, the specified actions from Module A become available within the **[Agent](/concepts/profiles-agents)** context of Module B when it runs. Module B's agent can then invoke these actions as if they were its own.
    *   **Execution:** Importantly, when Module B invokes a provided Action, the Action code still executes securely within an isolated Docker container configured with the *Provider* Module's (Module A's) environment, dependencies, and workspace context. The Receiver (Module B) only gets the *result*.
    *   **Use Case:** Allowing one Module to leverage the specialized capabilities of another (e.g., a `WebSearch` Module providing a `search(query)` action to a `ResearchAssistant` Module).
...
2.  **`WORKSPACE`** <FolderGitIcon className="inline-block h-5 w-5 align-text-bottom" />
    ...
    *   **Access:** Actions executed by Module B can then *read* files from this mounted submodule path (`/repo/workspaces/{provider_module_id}/...`). They cannot directly write back to the provider's workspace.
...
flowchart TB
    ...
    ModuleA -->|"Provide ACTION 'do_thing'"| ModuleB
...
```
*Diagram: Module A provides an Action and its Workspace to Module B. Module B's Agent can call `do_thing` (executed in A's context), and its Actions can read from `/repo/workspaces/module_a_id`.*
...
The Provide mechanism ensures controlled sharing. A Module only gains access to what another Module explicitly provides, and the execution context (for Actions) remains tied to the Provider, enhancing security and encapsulation.
</Callout>
