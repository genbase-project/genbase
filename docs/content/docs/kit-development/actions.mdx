...
description: Learn how to implement executable Python functions (Actions) within your Genbase Kit.
---
...
**[Actions](/concepts/actions-execution)** are the core executable logic units within a Genbase Kit. They are Python functions that **[Agents](/concepts/profiles-agents)** can invoke to perform tasks beyond simple LLM interaction, such as interacting with databases, calling external APIs, or manipulating files in the Module's **[Workspace](/concepts/workspace-resources)**.

## Defining Actions

1.  **Location:** Place your Python code within the `actions/` directory at the root of your Kit.
2.  **Organization:** You can organize your code into multiple `.py` files (modules). An `actions/__init__.py` file is common for simple or central actions.
...
4.  **Parameters & Return Values:**
    ...
    *   Use type hints for parameters and return types. These hints, along with the docstring, are used by Genbase to generate the schema that the LLM uses to call your action correctly.
...
5.  **Docstrings:** Write clear docstrings for your functions. The first line/paragraph is used as the action's `description` in `kit.yaml` (unless explicitly overridden there). Use standard docstring formats (like Google or NumPy style) to document parameters (`:param name: description`) so Genbase can potentially extract parameter descriptions for the schema.

## Declaring Actions in `kit.yaml`

For an action function to be usable, it must be declared in your `kit.yaml` file, either within a `profiles` section or the `provide.actions` section.
  ...
  some_profile:
    ...
    actions:
      - name: "run_query" # Name used by Agent/LLM
        path: "db_tools:execute_select" # Corresponds to execute_select() in actions/db_tools.py
        description: "Executes a read-only SQL SELECT query against the database."
      - name: "simple_task"
        path: "perform_simple_task" # Assumes perform_simple_task() in actions/__init__.py
        description: "Performs a basic task defined directly in __init__.py."

provide:
  actions:
    - name: "get_data_summary"
      path: "data_processor:summarize" # summarize() in actions/data_processor.py
...
*   `path`: Locates the function.
    ...
    *   `function_name`: For functions defined directly in `actions/__init__.py`.
*   `description`: Essential for the LLM to understand *what* the action does and *when* to use it. Be clear and specific.

## Execution Environment <TerminalIcon className="inline-block h-5 w-5 align-text-bottom" />

Remember that Kit Actions run inside isolated Docker containers. Your code needs to operate within this context:
...
*   **Action Code Access:** The Kit's `actions/` directory is mounted read-only at `/actions`. You can import helper functions or modules from within this directory using relative imports if structured correctly as a Python package (using `__init__.py` files).
*   **Networking:** Containers run in Docker's default bridge network. They can typically access the host machine and external internet resources. If an Action needs to expose a service, define a **[Port](/kit-development/kit-yaml#detailed-sections)** in `kit.yaml`. The mapped *host* port will be available as an environment variable like `PORT_{YOUR_PORT_NAME}` inside the container.

## Example Action (`actions/file_utils.py`)

```python
# actions/file_utils.py
import os
