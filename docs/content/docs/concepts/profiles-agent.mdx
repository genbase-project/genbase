...
*   **Specifies:**
    ...
    *   Available **Actions** for that mode.
...
graph LR
    ...
    subgraph "Profiles"
        PA --> AgentA["Agent X"]
        PA --> ActionsA["Actions: check, configure"]
        PA --> InstructionsA["Instructions: init_guide.md"]

        PB --> AgentB["Agent Y"]
        PB --> ActionsB["Actions: monitor, cleanup"]
    ...
    classDef action fill:#FFD700,stroke:#FFD700,color:#333,rx:5px,ry:5px
    ...
    class ActionsA,ActionsB action
    class InstructionsA,InstructionsB instruction
```
*Diagram: A Module can offer multiple Profiles, each linking to specific Agents, Actions, and Instructions.*

## Agents <BotIcon className="inline-block h-5 w-5 align-text-bottom" />

An Agent is the **intelligent controller** that executes the logic for a specific Profile. It uses LLMs and Actions to respond to user input within that Profile's context.

*   **Purpose:** Manage the conversation flow, interpret user requests, decide on actions (call LLM, run code), and generate responses.
...
*   **Key Tasks:**
    ...
    *   Execute available Actions (`ActionService`).
...
```
*Diagram: Simplified chat interaction flow involving the User, UI, Engine, Agent, LLM, and Action Service.*
