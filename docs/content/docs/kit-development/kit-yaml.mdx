...
dependencies:            # Optional: List of Python packages required by actions
  - requests
  - beautifulsoup4
...
# --- Runtime & Execution ---
image: "python:3.11-slim" # Optional: Base Docker image for action execution (default: python:3.11-slim)
ports:                   # Optional: Ports to map from the action container to the host
...
<Accordions>
  <Accordion title="Metadata & Dependencies">
    ...
    *   `dependencies` (Optional, List of Strings): A list of Python package names required by the Kit's **[Actions](/kit-development/actions)**. These will be installed via `pip` in the Action execution container. Format should match `requirements.txt` entries. Example: `['requests', 'psycopg2-binary>=2.9']`.
    *   `environment` (Optional, List of Objects): Defines environment variables expected by the Kit's Actions or Agents. Users will be prompted to provide values when creating a Module from this Kit.
    ...
    *   `image` (Optional, String): Specifies the base Docker image used for running the Kit's **[Actions](/concepts/actions-execution)**. Defaults to `python:3.11-slim`. Choose an image that meets your Action's runtime needs.
    *   `ports` (Optional, List of Objects): Defines network ports that should be mapped from the Action execution container to the host machine.
        ...
        *   `port` (Required, Integer): The port number *inside* the container that your Action listens on. Genbase will find an available port on the host and map it to this container port.
  </Accordion>
  ...
  <Accordion title="Profiles">
    ...
    profiles:
      ...
      initialize:
        ...
        actions:                # Optional: List of actions available in this profile
          - name: "check_system"
          ...
          - name: "apply_config"
            path: "configure" # Assumes 'configure()' function in 'actions/__init__.py'
      ...
      maintain:
        ...
        actions:
          ...
          # ... more actions specific to maintenance
    ```
    ...
    *   `actions` (Optional, List of Objects): Defines which **[Kit Actions](/concepts/actions-execution)** are available to the `agent` when operating under this specific `profile`.
        *   `name` (Required, String): The name the agent will use to refer to this action (e.g., in tool calls).
        *   `path` (Required, String): Specifies the action function. Format: `filename:function_name` (e.g., `utils:parse_data`) or just `function_name` (implies the function is in `actions/__init__.py`).
        *   `description` (Required, String): A clear description for the LLM explaining what the action does and potentially how/when to use it.
  </Accordion>
  ...
  <Accordion title="Provide">
    ...
    ```yaml
    provide:
      actions:          # Optional: Actions this kit offers for others to use
        - name: "get_summary"
      ...
      instructions:     # Optional: Instructions provided as context to consumers
        - name: "api_usage_guide"
          ...
          description: "How to effectively use the provided actions."

      workspace:        # Optional: Allow sharing the module's workspace
        description: "Provides access to the processed data files."
    ```
    *   `actions` (Optional, List of Objects): Actions made available to consuming modules. Structure is identical to actions within `profiles`. When a consuming module calls a provided action, it executes in the *provider's* context.
    ...
    workspace:
      files:          # Optional: List of files/dirs from the kit's 'workspace/' dir
        ...
        - path: "scripts/init.py"
          description: "Initial setup script (can be run by an action)."
...
Genbase validates the `kit.yaml` file when a Kit is installed or uploaded. Ensure your YAML is well-formed and adheres to this specification. Referencing non-existent files (in `instructions`, `workspace.files`, `actions`) will likely cause errors during Module creation or Action execution.
</Callout>
