...
from engine.const import WORKSPACE_BASE_DIR
...
class AgentUtils:
    ...
    def __init__(self, module_service: ModuleService, workspace_service: WorkspaceService, module_id: str, profile: str):
        """
        ...
        # Get module metadata to access workspace info
        self.module_metadata = self.module_service.get_module_metadata(module_id)
        self.workspace_path =  WORKSPACE_BASE_DIR / self.module_metadata.workspace_name

    def read_file(self, relative_path: str) -> Optional[str]:
        """
        Read contents of a file relative to the workspace root.
        ...
        """
        try:
            file_path = Path(self.workspace_path) / relative_path
    ...
    def read_files(self, relative_paths: List[str]) -> Dict[str, Optional[str]]:
        """
        Read contents of multiple files relative to the workspace root
        
        Args:
            relative_paths: List of paths relative to workspace root
    ...
    def list_files(self, relative_path: str = "") -> List[Path]:
        """
        List all files in a directory relative to the workspace root
        
        Args:
            relative_path: Path relative to workspace root. If empty, lists from root
                
        Returns:
            List[Path]: List of Path objects for all files, with paths relative to workspace root
                
        Raises:
            Exception: On directory access errors
        """
        try:
            dir_path = Path(self.workspace_path) / relative_path
            ...
            # Convert absolute paths to paths relative to workspace_path
            return [p.relative_to(self.workspace_path) for p in all_files]
        except Exception as e:
    ...
    def get_workspace_tree(self, path: Optional[Path]=None) -> str:
        ...
        if path:
            dir_path = WORKSPACE_BASE_DIR / self.module_metadata.workspace_name / path
        else:
            dir_path = WORKSPACE_BASE_DIR / self.module_metadata.workspace_name
            
        logger.info(f"Getting tree for workspace: {dir_path}")
