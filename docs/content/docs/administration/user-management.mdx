...
*(Image: Screenshot of the User Management page in Studio, showing the table of users, status/role badges, and action buttons.)*
...
2.  **Open Dialog:** Click the Edit icon (<EditIcon className="inline-block h-4 w-4" />) in the "Actions" column for that user.
...
2.  **Initiate Deletion:** Click the Delete icon (<Trash2Icon className="inline-block h-4 w-4" />) in the "Actions" column.
3.  **Confirm:** An alert dialog will appear, warning you that the action is irreversible. Read the warning carefully.
4.  **Confirm Action:** Click the "Delete User" (or similarly labeled confirmation) button, often styled distinctively (e.g., red).

    ![Delete User Confirmation Dialog]( /images/studio-delete-user-confirm-dialog.png)
    *(Image: Screenshot of the confirmation dialog shown before deleting a user, emphasizing the permanent nature of the action.)*
