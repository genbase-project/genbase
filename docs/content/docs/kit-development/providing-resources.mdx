...
description: Learn how to define shared Actions, Workspaces, and Instructions in your Kit's `provide` section.
---
...
provide:
  # --- Actions shared by this Kit ---
  actions:
    - name: "lookup_user_id" # Name the consuming agent will use
      path: "user_service:get_id_by_email" # Function in actions/user_service.py
      description: "Looks up a user ID based on their email address."
    - name: "validate_token"
      path: "auth:verify_token" # Function in actions/auth.py
  ...
  instructions:
    - name: "api_guidelines"
      ...
      description: "Best practices for interacting with this Kit's provided actions."
...
1.  **`actions`** <ZapIcon className="inline-block h-4 w-4 align-text-bottom" /> (Optional, List of Objects):
    *   Declares **[Kit Actions](/concepts/actions-execution)** that consuming Modules can invoke.
    *   The structure for each action (`name`, `path`, `description`) is identical to defining actions within `profiles`.
    *   **Crucially:** When a consuming Module's Agent calls a provided action, the action code executes in an isolated Docker container using the **Provider Module's** environment variables, dependencies, and workspace context. Only the result is returned to the consuming Agent.
...
2.  **`instructions`** <FileTextIcon className="inline-block h-4 w-4 align-text-bottom" /> (Optional, List of Objects):
    ...
    *   This information can be made available to the *consuming* Module's Agent to help it understand how to use the provided Actions or interpret the provided Workspace content.
...
*   **Consuming Actions:** Once Module A provides an Action (e.g., `lookup_user_id`) to Module B, that action name becomes available as a potential tool call for Module B's Agent when it interacts with its Profiles. The Agent's `set_context` method (potentially via `IncludeOptions(provided_actions=True)`) can be configured to include these provided actions in the list of tools passed to the LLM.
*   **Consuming Workspace:** Once Module A provides its Workspace to Module B, any Action executed by Module B can access the files within Module A's workspace by reading from the path `/repo/workspaces/{module_a_id}/...` inside its Docker execution container.
...
*   Only `provide` Actions and Instructions that make sense for external consumption. Keep internal helper functions private.
*   Ensure provided Actions have clear descriptions for the consuming LLM/Agent.
*   Document the purpose and structure of a provided Workspace clearly in the `provide.workspace.description` or a README.
*   Remember that provided Actions execute within the *Provider's* context – they have access to the Provider's environment variables and workspace, not the Consumer's.
</Callout>
