...
*   **Context for Provided Resources:** Explain the purpose or usage guidelines for Actions or Workspaces shared via the `provide` section in `kit.yaml`.
...
2. Recommend appropriate maintenance actions (VACUUM, ANALYZE) based on stats or user request.
...
provide:
  ...
  instructions: # Instructions explaining the provided resources
    - name: "provider_usage"
      ...
      description: "Explains how consuming modules should use the provided actions."
```
