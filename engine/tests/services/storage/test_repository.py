...
def repo_service(tmp_path: Path) -> RepoService:
    ...
    base_path.mkdir()
    return RepoService(base_path=base_path)
...
@pytest.fixture
def create_test_repo(repo_service: RepoService, create_zip_content: bytes):
...
@pytest.fixture
def create_parent_child_repos(repo_service: RepoService):
...
class TestRepoService:
    ...
    def test_get_repo_path(self, repo_service: RepoService):
        repo_name = "my_repo"
        expected_path = repo_service.base_path / repo_name
        assert repo_service.get_repo_path(repo_name) == expected_path

    def test_create_repository_success(self, repo_service: RepoService, create_zip_content: bytes):
        repo_name = "new_repo"
        content_file = io.BytesIO(create_zip_content)
        result = repo_service.create_repository(
            repo_name=repo_name,
    ...
    def test_create_repository_already_exists(self, repo_service: RepoService, create_test_repo):
        repo_name, _, _ = create_test_repo
        content_file = io.BytesIO(b"dummy content") # Content doesn't matter here
        with pytest.raises(RepoExistsError, match=f"Repository {repo_name} already exists"):
    ...
    def test_list_repositories_empty(self, repo_service: RepoService):
        assert repo_service.list_repositories() == []

    def test_list_repositories_single(self, repo_service: RepoService, create_test_repo):
        repo_name, _, _ = create_test_repo
        assert repo_service.list_repositories() == [repo_name]

    def test_list_repositories_multiple(self, repo_service: RepoService, create_zip_content: bytes):
        repo_name1 = "repo1"
        repo_name2 = "repo2"
        repo_service.create_repository(repo_name1, io.BytesIO(create_zip_content), "r1.zip", extract_zip)
        repo_service.create_repository(repo_name2, io.BytesIO(create_zip_content), "r2.zip", extract_zip)
        assert sorted(repo_service.list_repositories()) == sorted([repo_name1, repo_name2])

    def test_list_files_success(self, repo_service: RepoService, create_test_repo):
    ...
    def test_list_files_repo_not_found(self, repo_service: RepoService):
        with pytest.raises(RepoNotFoundError):
            repo_service.list_files("non_existent_repo")

    def test_delete_repository_success(self, repo_service: RepoService, create_test_repo):
        repo_name, repo_path, _ = create_test_repo
        assert repo_path.exists()
        repo_service.delete_repository(repo_name)
        assert not repo_path.exists()

    def test_delete_repository_not_found(self, repo_service: RepoService):
        with pytest.raises(RepoNotFoundError):
            repo_service.delete_repository("non_existent_repo")

    def test_commit_changes_no_changes(self, repo_service: RepoService, create_test_repo):
        repo_name, _, _ = create_test_repo
    ...
    def test_commit_changes_with_changes(self, repo_service: RepoService, create_test_repo):
        repo_name, repo_path, _ = create_test_repo
    ...
    def test_commit_changes_repo_not_found(self, repo_service: RepoService):
         commit_info = CommitInfo(commit_message="Test commit")
         with pytest.raises(RepoNotFoundError):
            repo_service.commit_changes("non_existent_repo", commit_info)

    def test_update_file_create_new(self, repo_service: RepoService, create_test_repo):
        repo_name, repo_path, _ = create_test_repo
    ...
    def test_update_file_update_existing(self, repo_service: RepoService, create_test_repo):
        repo_name, repo_path, _ = create_test_repo
    ...
    def test_update_file_invalid_path(self, repo_service: RepoService, create_test_repo):
        repo_name, _, _ = create_test_repo
        with pytest.raises(RepositoryError, match="Invalid file path"):
            repo_service.update_file(repo_name, "../outside.txt", "danger", is_safe_path)

    def test_update_file_repo_not_found(self, repo_service: RepoService):
        with pytest.raises(RepoNotFoundError):
            repo_service.update_file("non_existent_repo", "file.txt", "content", is_safe_path)

    def test_get_active_branch(self, repo_service: RepoService, create_test_repo):
        repo_name, _, _ = create_test_repo
        branch = repo_service.get_active_branch(repo_name)
        # Default branch is usually 'master' or 'main'
        assert branch in ["master", "main"]

    def test_get_active_branch_repo_not_found(self, repo_service: RepoService):
        with pytest.raises(RepoNotFoundError):
            repo_service.get_active_branch("non_existent_repo")

    def test_add_submodule_success(self, repo_service: RepoService, create_parent_child_repos):
    ...
    def test_add_submodule_parent_not_found(self, repo_service: RepoService, create_parent_child_repos):
        _, child_name = create_parent_child_repos
        with pytest.raises(RepoNotFoundError, match="Parent repository"):
            repo_service.add_submodule("fake_parent", child_name, "libs/child")

    def test_add_submodule_child_not_found(self, repo_service: RepoService, create_parent_child_repos):
        parent_name, _ = create_parent_child_repos
        with pytest.raises(RepoNotFoundError, match="Child repository"):
            repo_service.add_submodule(parent_name, "fake_child", "libs/child")



    def test_remove_submodule_repo_not_found(self, repo_service: RepoService):
        with pytest.raises(RepoNotFoundError):
            repo_service.remove_submodule("non_existent_repo", "some/path")

    def test_remove_submodule_not_a_submodule(self, repo_service: RepoService, create_test_repo):
        repo_name, repo_path, _ = create_test_repo
        # 'subdir' exists but is not a submodule
        with pytest.raises(RepositoryError, match="'subdir' is not a submodule"):
