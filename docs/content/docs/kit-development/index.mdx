...
2.  **Well-Defined Interface:** Expose functionality through clear **[Profiles](/concepts/profiles-agents)** and well-documented **[Actions](/concepts/actions-execution)**. Think about how other modules or users will interact with it.
3.  **Encapsulation:** Keep internal logic hidden where possible. Expose necessary functionality via Actions and manage internal state within the Module's **[Workspace](/concepts/workspace-resources)** or Profile Store.
...
5.  **Reusability:** Design Actions and logic to be generally applicable within the Kit's domain, avoiding hardcoding project-specific details.
6.  **Resource Sharing (Provide):** Clearly define what the Kit *provides* (`provide` section in `kit.yaml`) if it's intended to share functionality (Actions) or data (Workspace) with other Modules.
...
Avoid creating monolithic Kits that try to do everything. Instead, break down complex problems into smaller, composable Kits that can work together through defined interfaces (Profiles, Actions, Provide).
</Callout>
...
1.  **Planning:** Define the Kit's purpose, inputs, outputs, actions, and configuration.
...
4.  **Implementing Actions:** Write the core Python logic in the `actions/` directory. ([See Actions](/kit-development/actions))
5.  **(Optional) Implementing Agents:** Create custom agent logic if needed. ([See Agents](/kit-development/agents))
6.  **Defining Profiles:** Link agents and actions for specific interaction modes in `kit.yaml`. ([See Profiles](/kit-development/profiles))
...
11. **Testing:** Install the Kit locally, create a Module, and test its Profiles and Actions thoroughly.
...
<Cards>
  ...
  <Card href="/docs/kit-development/actions" title="Writing Actions" />
