...
import { PackageSearchIcon, PackageIcon, DownloadIcon, LayersIcon } from 'lucide-react';
...
1.  **Navigate:** Click the "Registry" icon (<PackageIcon className="inline-block h-4 w-4" />) in the far-left navigation sidebar of Genbase Studio.
...
*   **Tabs (Content Varies by Kit):**
    ...
    *   **Profiles:** Details about each Profile the Kit offers, including the assigned Agent, available Tools, and associated Instructions.
    *   **Workspace:** Lists the initial files and directories included in the Kit's workspace template.
    *   **Provide:** Shows the Tools, Instructions, or Workspace access the Kit is designed to provide to other modules.
